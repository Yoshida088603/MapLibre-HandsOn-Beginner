...
- **街区基準点等データ**: 国土数値情報の街区基準点等 CSV（例: H_01101 札幌市中央区）を PMTiles に変換して表示（青い円で表示。札幌付近にパンすると見える）
...
- 背景は OSM 日本、土地活用は紫色の円/塗り。単体確認用に TH_23521 をオレンジで表示するレイヤもあり。**街区基準点（H_01101）**は青い円で表示（札幌市中央区付近にパンすると見える。クリックで所在地・基準点コード等のポップアップ）。座標列は土地活用と同様に Y,X 並びで解釈している。
